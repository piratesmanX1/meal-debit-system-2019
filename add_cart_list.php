...
$CHECKQUAN = "SELECT *
              ...
              WHERE transaction_record.order_id = '".$_SESSION["order_id"]."' AND transaction_record.meal_id = '".$_POST["meal_id"]."'";
...
if (mysqli_num_rows($CHECKQUANQ) < 1) {
  ...
  $sql = "INSERT INTO transaction_record(meal_brand_id, meal_quantity_cart, meal_id, order_id)
          VALUES('".$_SESSION["brand_id"]."', '1', '".$_POST["meal_id"]."', '".$_SESSION["order_id"]."')";
  if(mysqli_query($con, $sql)) {
       echo 'Notice: Meal ID '.$_POST["meal_id"].' inserted into Order ID: '.$_SESSION["order_id"].'.';
  }
} else {
  if ($row = mysqli_fetch_array($CHECKQUANQ)) {
    // now, taking in the value and compare //
    if (($row["meal_quantity_cart"] + 1) <= $row["meal_quantity"]) {
      ...
      if (mysqli_num_rows($CHECKMEALQ) < 1) {
        ...
        $sql = "UPDATE transaction_record SET meal_quantity_cart = (meal_quantity_cart + 1)
                WHERE meal_id = '".$_POST["meal_id"]."' AND order_id = '".$_SESSION["order_id"]."'";
        if(mysqli_query($con, $sql)) {
             echo 'Notice: Meal quantity of Meal ID '.$_POST["meal_id"].' in Order ID: '.$_SESSION["order_id"].' is updated.';
        }
      }
    } else {
