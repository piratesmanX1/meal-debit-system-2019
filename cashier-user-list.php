...
<!-- Title of the table -->
<!-- Cashier List: Active Section -->
<div class="row x_title">
  <div class="col-md-6">
    <h3>Cashier <small>Active </small></h3>
  </div>
</div>
...
<?php
 ...
 $CASHIERA = "SELECT *
              FROM user INNER JOIN cashier
              ON user.user_id = cashier.user_id
              WHERE user.status = 1 AND user.active = 1
              LIMIT $page_count, 10";
 $CASHIERAQ = mysqli_query($con, $CASHIERA);
 $output_active .= '
      <div class="">
 ...
 $rows = mysqli_num_rows($CASHIERAQ);
 if($rows > 0) {
      while($row = mysqli_fetch_array($CASHIERAQ)) {
           ...
           $output_active .= '
                <tr>
                     ...
                     <td class="gender" data-id3="'.$row["user_id"].'">'.$gender.'</td>
                     <td class="dob" data-id4="'.$row["user_id"].'">'.$dob.'</td>
                     <td class="last_login" data-id5="'.$row["user_id"].'">'.$last_login.'</td>
                </tr>
           ';
      }
 } else {
...
<br>
<br>
<!-- Cashier List: Inctive Section -->
<div class="row x_title">
  <div class="col-md-6">
    <h3>Cashier <small>Inactive </small></h3>
  </div>
</div>
...
<?php
 ...
 $CASHIERI = "SELECT *
              FROM user INNER JOIN cashier
              ON user.user_id = cashier.user_id
              WHERE user.status = 1 AND user.active = 0
              LIMIT $page_count, 10";
 $CASHIERIQ = mysqli_query($con, $CASHIERI);
 $output_inactive .= '
      <div class="">
           <table class="table table-bordered">
                <tr>
 ...
 $rows = mysqli_num_rows($CASHIERIQ);
 if($rows > 0) {
      while($row = mysqli_fetch_array($CASHIERIQ)) {
           // now defining the gender based on the value //
           if (($row["gender"]) == 0) {
             $gender = "MALE";
           } else if (($row["gender"]) == 1) {
...
if ($total_usea > $total_usei) {
 ...
 echo '<div class="center">
      <div class="pagination" id="page-div">
        <a class="page" onclick="casTable(this)" name="table-page" value="';
        ...
        echo '" id="page-'.$previous_page.'">&laquo;</a>
          ...
          echo '" onclick="casTable(this)" name="table-page" id="page-1" value="1"> 1 </a>';
 for ($n = 2; $n <= $page_numb; $n++) {
   ...
   echo '" onclick="casTable(this)" name="table-page" id="page-'.$n.'" value="'.$n.'">'.$n.'</a>';
 }
 echo '<a class="page" name="table-page" onclick="casTable(this)" value="';
     ...
     <div class="logmod__container">
       ...
       <div class="logmod__tab lgm-1" id="ver-form-dis">
         ...
         <div class="logmod__form">
             ...
             <div class="simform__actions">
               <input disabled class="sumbit" name="submit-list-dis-cas" style="font-family:oswald!important;" type="submit" id="submit-dis" value="Disable" />
       ...
       <form method="POST" action="">
         ...
         <div class="logmod__form">
             ...
             <div class="simform__actions">
               <input disabled class="sumbit" name="submit-list-ena-cas" style="font-family:oswald!important;" type="submit" id="submit-r" value="Enable" />
